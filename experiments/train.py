...
# This file is adapted from https://github.com/fla-org/flame/blob/main/flame/train.py
...
def init_dist(back_end="nccl"):
    ...
    os.environ['LOCAL_WORLD_SIZE']=NGPUS_PER_NODE
    if local_rank != 0:
        suppress_wandb()
...
def main(job_config: JobConfig):
    ...
    if int(os.environ['LOCAL_RANK']) == 0:
        wandb_entity = job_config.wandb.entity
        wandb_project = job_config.wandb.project
        wandb_mode = job_config.wandb.mode

        wandb_name = AutoConfig.from_pretrained(job_config.model.config).name
    ...
    with maybe_enable_profiling(
        ...
        while train_state.step < job_config.training.steps:
            ...
            for _ in range(job_config.training.gradient_accumulation_steps):
                ...
                losses.append(loss)
                attn_fracs.append(output.attn_frac)
            loss = sum(losses)
            attn_frac = sum(attn_fracs) / job_config.training.gradient_accumulation_steps
            ...
            if metric_logger.should_log(train_state.step):
                if (
                    ...
                    attn_frac = dist_utils.dist_mean(attn_frac, world_mesh['dp_cp'])
                else:
                    # Scale back the loss before logging
                    global_avg_loss = global_max_loss = loss.item()
                    attn_frac = attn_frac.item()
                ...
                logger.info(
                    f"{color.blue}lr: {last_lr:.4e} gnorm: {grad_norm:5.2f}, attn fract: {attn_frac:5.3f}"
