...
def _copy_to_param(param: torch.Tensor, value: torch.Tensor) -> None:
    """Copy value into param, handling DTensor (replicated or sharded) correctly."""
    if _dtensor_available and isinstance(param, DTensor):
        dist_val = distribute_tensor(value.to(param._local_tensor.device), param.device_mesh, param.placements)
        param.copy_(dist_val)
    else:
        param.copy_(value)
...
from nats.layers.natsl_attn_mamba2 import NeuralAttentionSearchLinearAttnMamba
...
class Mamba2Cache:
    ...
    def __init__(
        ...
        self.nats_cache: dict[int, NAtSLayerCache] = {}
...
class NAtSMamba2Block(GradientCheckpointingLayer):

    def __init__(self, config: NATSMamba2Config, layer_idx):
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
        self.residual_in_fp32 = config.residual_in_fp32
        if layer_idx in config.mamba_layers:
            self.is_mamba_block = True
            ...
            self.mixer = Mamba2(
                ...
                hidden_act=config.mamba_hidden_act,
            ...
            self.is_mamba_block = False
            self.attn = NeuralAttentionSearchLinearAttnMamba(
                hidden_size=config.hidden_size,
                expand_v=config.expand_v,
                head_dim=config.head_dim_nats,
                ...
                decay_for_non_mamba_blocks=config.decay_for_non_mamba_blocks,
...
class NAtSMamba2PreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = NATSMamba2Config
    base_model_prefix = "backbone"
    _no_split_modules = ["NAtSMamba2Block"]
    ...
    def _init_weights(
        ...
        if (isinstance(module, Mamba2) and next(module.parameters()).device.type != 'meta'):
            ...
            with torch.no_grad():
                ...
                A = torch.arange(1, module.num_heads + 1)
                #module.A_log.copy_(torch.log(A))
                _copy_to_param(module.A_log, torch.log(A))
            ...
            with torch.no_grad():
                ...
                """
                dt = torch.exp(
                    torch.rand(module.dt_bias.shape)
                ...
                _copy_to_param(module.dt_bias, inv_dt)
                module.dt_bias._no_reinit = True

        elif (isinstance(module, NeuralAttentionSearchLinearAttnMamba) and next(module.parameters()).device.type != 'meta'):
            with torch.no_grad():
                ...
                # Inverse of softplus: https://github.com/pytorch/pytorch/issues/72759
                inv_dt = dt + torch.log(-torch.expm1(-dt))
                module.dt_bias.copy_(nn.init.ones_(module.dt_bias))
...
class Mamba2CausalLMOutput(ModelOutput):
    ...
    cache_params: Optional[Mamba2Cache] = None
    hidden_states: Optional[Tuple[torch.FloatTensor]] = None
    attn_frac: float=None


class NAtSMamba2Model(NAtSMamba2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        self.embeddings = nn.Embedding(config.vocab_size, config.hidden_size)
        self.layers = nn.ModuleList([NAtSMamba2Block(config, layer_idx=idx) for idx in range(config.num_hidden_layers)])
...
class NAtSMamba2ForCausalLM(NAtSMamba2PreTrainedModel, FLAGenerationMixin):
    _tied_weights_keys = []

    def __init__(self, config):
        super().__init__(config)
        self.backbone = NAtSMamba2Model(config)
    ...
    def forward(
        ...
        **kwargs,  # for now we need this for generation
        ...
        all_attn_fraction = [layer.attn.attn_fraction for layer in self.backbone.layers if hasattr(layer, 'attn') and isinstance(layer.attn, NeuralAttentionSearchLinearAttnMamba)]
        if len(all_attn_fraction) >0:
            all_attn_fraction = torch.mean(torch.cat(all_attn_fraction))
        else:
            all_attn_fraction = torch.zeros(1).squeeze().cuda()
