...
def solve_tril_16x16_kernel(
    ...
    A = A + (bos*GNAtS + i_gnats) * BT
    Ai = Ai + (bos*GNAtS + i_gnats) * 16

    offset = (i_t * 16) % BT
    if not USE_TMA:
        p_A = tl.make_block_ptr(A, (T, BT), (GNAtS*BT, 1), (i_t * 16, offset), (16, 16), (1, 0))
        # [16, 16]
        b_A = tl.load(p_A, boundary_check=(0, 1)).to(tl.float32)
    else:
        desc = make_tensor_descriptor(A, [T, BT], [GNAtS*BT, 1], [16, 16])
        desc_o = make_tensor_descriptor(Ai, [T, 16], [GNAtS*16, 1], [16, 16])
    ...
    b_A += m_I
    if not USE_TMA:
        p_Ai = tl.make_block_ptr(Ai, (T, 16), (GNAtS*16, 1), (i_t * 16, 0), (16, 16), (1, 0))
...
def merge_16x16_to_32x32_inverse_kernel(
    ...
    # TODO chekc if we need to transform i_t into int64!!!

    if not USE_TMA:
        p_A_11 = tl.make_block_ptr(A, (T, BT), (GNAtS*BT, 1), (i_t * BT, 0), (16, 16), (1, 0))
        p_A_22 = tl.make_block_ptr(A, (T, BT), (GNAtS*BT, 1), (i_t * BT + 16, 16), (16, 16), (1, 0))
        b_Ai_11 = tl.load(p_A_11, boundary_check=(0, 1)).to(tl.float32)
        b_Ai_22 = tl.load(p_A_22, boundary_check=(0, 1)).to(tl.float32)
    else:
        desc = make_tensor_descriptor(A, [T, BT], [GNAtS*BT, 1], [16, 16])
        desc_o = make_tensor_descriptor(Ai, [T, BT], [GNAtS*BT, 1], [16, 16])
    ...
    for i in range(2, min(16, T - i_t * BT)):
        b_a_11 = -tl.load(A + (i_t * BT + i) * GNAtS*BT + o_i)
        b_a_11 += tl.sum(b_a_11[:, None] * b_Ai_11, 0)
        b_Ai_11 = tl.where((o_i == i)[:, None], b_a_11, b_Ai_11)
    for i in range(16 + 2, min(32, T - i_t * BT)):
        b_a_22 = -tl.load(A + (i_t * BT + i) * GNAtS*BT + o_i + 16)
    ...
    if not USE_TMA:
        p_A_21 = tl.make_block_ptr(A, (T, BT), (GNAtS*BT, 1), (i_t * BT + 16, 0), (16, 16), (1, 0))
    ...
    if not USE_TMA:
        p_Ai_11 = tl.make_block_ptr(Ai, (T, BT), (GNAtS*BT, 1), (i_t * BT, 0), (16, 16), (1, 0))
        p_Ai_21 = tl.make_block_ptr(Ai, (T, BT), (GNAtS*BT, 1), (i_t * BT + 16, 0), (16, 16), (1, 0))
        p_Ai_22 = tl.make_block_ptr(Ai, (T, BT), (GNAtS*BT, 1), (i_t * BT + 16, 16), (16, 16), (1, 0))
...
def merge_16x16_to_64x64_inverse_kernel(
    ...
    if not USE_TMA:
        p_A_11 = tl.make_block_ptr(A, (T, BT), (GNAtS*BT, 1), (i_t * BT, 0), (16, 16), (1, 0))
        p_A_22 = tl.make_block_ptr(A, (T, BT), (GNAtS*BT, 1), (i_t * BT + 16, 16), (16, 16), (1, 0))
        p_A_33 = tl.make_block_ptr(A, (T, BT), (GNAtS*BT, 1), (i_t * BT + 32, 32), (16, 16), (1, 0))
        p_A_44 = tl.make_block_ptr(A, (T, BT), (GNAtS*BT, 1), (i_t * BT + 48, 48), (16, 16), (1, 0))
        b_Ai_11 = tl.load(p_A_11, boundary_check=(0, 1)).to(tl.float32)
        b_Ai_22 = tl.load(p_A_22, boundary_check=(0, 1)).to(tl.float32)
    ...
    for i in range(32 + 2, min(48, T - i_t * BT)):
        b_a_33 = -tl.load(A + (i_t * BT + i) * GNAtS*BT + o_i + 32)
        b_a_33 += tl.sum(b_a_33[:, None] * b_Ai_33, 0)
        b_Ai_33 = tl.where((o_i == i - 32)[:, None], b_a_33, b_Ai_33)
    for i in range(48 + 2, min(64, T - i_t * BT)):
        b_a_44 = -tl.load(A + (i_t * BT + i) * GNAtS*BT + o_i + 48)
    ...
    if not USE_TMA:
        p_A_21 = tl.make_block_ptr(A, (T, BT), (GNAtS*BT, 1), (i_t * BT + 16, 0), (16, 16), (1, 0))
        p_A_31 = tl.make_block_ptr(A, (T, BT), (GNAtS*BT, 1), (i_t * BT + 32, 0), (16, 16), (1, 0))
        p_A_32 = tl.make_block_ptr(A, (T, BT), (GNAtS*BT, 1), (i_t * BT + 32, 16), (16, 16), (1, 0))
        p_A_41 = tl.make_block_ptr(A, (T, BT), (GNAtS*BT, 1), (i_t * BT + 48, 0), (16, 16), (1, 0))
        p_A_42 = tl.make_block_ptr(A, (T, BT), (GNAtS*BT, 1), (i_t * BT + 48, 16), (16, 16), (1, 0))
        p_A_43 = tl.make_block_ptr(A, (T, BT), (GNAtS*BT, 1), (i_t * BT + 48, 32), (16, 16), (1, 0))
        b_A_21 = tl.load(p_A_21, boundary_check=(0, 1)).to(tl.float32)
    ...
    if not USE_TMA:
        p_Ai_11 = tl.make_block_ptr(Ai, (T, BT), (GNAtS*BT, 1), (i_t * BT, 0), (16, 16), (1, 0))
        p_Ai_22 = tl.make_block_ptr(Ai, (T, BT), (GNAtS*BT, 1), (i_t * BT + 16, 16), (16, 16), (1, 0))
        p_Ai_33 = tl.make_block_ptr(Ai, (T, BT), (GNAtS*BT, 1), (i_t * BT + 32, 32), (16, 16), (1, 0))
        p_Ai_44 = tl.make_block_ptr(Ai, (T, BT), (GNAtS*BT, 1), (i_t * BT + 48, 48), (16, 16), (1, 0))
        p_Ai_21 = tl.make_block_ptr(Ai, (T, BT), (GNAtS*BT, 1), (i_t * BT + 16, 0), (16, 16), (1, 0))
        p_Ai_31 = tl.make_block_ptr(Ai, (T, BT), (GNAtS*BT, 1), (i_t * BT + 32, 0), (16, 16), (1, 0))
        p_Ai_32 = tl.make_block_ptr(Ai, (T, BT), (GNAtS*BT, 1), (i_t * BT + 32, 16), (16, 16), (1, 0))
        p_Ai_41 = tl.make_block_ptr(Ai, (T, BT), (GNAtS*BT, 1), (i_t * BT + 48, 0), (16, 16), (1, 0))
        p_Ai_42 = tl.make_block_ptr(Ai, (T, BT), (GNAtS*BT, 1), (i_t * BT + 48, 16), (16, 16), (1, 0))
        p_Ai_43 = tl.make_block_ptr(Ai, (T, BT), (GNAtS*BT, 1), (i_t * BT + 48, 32), (16, 16), (1, 0))
        tl.store(p_Ai_11, b_Ai_11.to(p_Ai_11.dtype.element_ty, fp_downcast_rounding="rtne"), boundary_check=(0, 1))
        tl.store(p_Ai_22, b_Ai_22.to(p_Ai_22.dtype.element_ty, fp_downcast_rounding="rtne"), boundary_check=(0, 1))
...
def solve_tril_nats(
    ...
    Args:
        A (torch.Tensor):
            [TNAtS, GNAtS, BT], where BT should only be 16, 32, or 64. Here we flatten the A from different
            batches and heads to only store the tokens that are required tyo be computed.
        chunk_indices_op_nats (torch.Tensor):
            [grid_size, 2], a value that reports the starting indices information for each worker
        n_nats_blocks (torch.Tensor):
    ...
    if BT == 16:
        ...
        raise ValueError(f"BT must be at most 64 but we get {BT}")
