...
# This function works the same as ops/common/chunk_o.py
# However, since the chunks formed by nats blocks do not align with the actual q blocks, we still need to
# TODO update: WE do not need this anymore haha!!! WE only need to mege this into q computation
# and check how many iters needs to be run beforehand!!!!
...
def chunk_fwd_kernel_o(
    ...
    if IS_VARLEN:
        i_tg = i_t
        i_b, i_t = tl.load(chunk_indices + i_t * 2).to(tl.int32), tl.load(chunk_indices + i_t * 2 + 1).to(tl.int32)
...
def compute_h_aligned(
        ...
        IS_VARLEN: tl.constexpr,
        N_NAtS_Chunk_PER_T: tl.constexpr,
):
    i_v, i_t, i_bh = tl.program_id(0), tl.program_id(1), tl.program_id(2)
    i_b, i_h = i_bh // H, i_bh % H
    ...
    # for k, h0, ht, they are stored as [B,T,H,D] and therefore we read them from i_b, i_h and
    # nats_block_indices
    ...
    # from the continous index tl.range(0,BT)

    # to read the corresponding w, v_new, and h, we need to read their starting

    start_nats_wvh_t = tl.load(nats_block_delta_offsets + i_b * HNAtS + i_hnats).to(tl.int32)
    end_nats_wvh_t = tl.load(nats_block_delta_offsets + i_b * HNAtS + i_hnats + 1).to(tl.int32)

    if IS_VARLEN:
        bos, eos = tl.load(cu_seqlens + i_b).to(tl.int32), tl.load(cu_seqlens + i_b + 1).to(tl.int32)
        T = eos - bos
        #NT = tl.cdiv(T, BT)
        #boh = tl.load(chunk_offsets + i_b).to(tl.int32)
        ...
        b_starting_idx = tl.load(starting_h_idx + i_t * HNAtS).to(tl.int32)
        i_tg = i_t
        NT = tl.cdiv(T, BT)
    ...
    g += (start_nats_wvh_t * BT) * GNAtS + i_gnats
    # TODO check if we need v_new before or after!!!! Now it seems to align with the current idx?
    ...
    T_NATS_Compute = tl.load(n_nats_blocks + (i_b * HNAtS + i_hnats) * N_TYPES + OFFSET_DELTA) * NAtS_BLOCK_SIZE
    # we only need to get the values until BT
    T_NATS_Compute = min(T_NATS_Compute, i_t * BT)
    stride_nats_block = N_TYPES * HNAtS
    h_new += (i_h + i_tg * H).to(tl.int64) * K * V

    b_starting_h_idx = tl.load(b_starting_idx + i_t * HNAtS).to(tl.int32)

    stride_v = GNAtS * V
    stride_g = GNAtS

    h += b_starting_h_idx * GNAtS * K * V
    # we only require this function if NATS_BLOCK_SIZE is greater than BT, so this should always hold?

    v_new += stride_v * b_starting_h_idx * BT // NAtS_BLOCK_SIZE
    g += stride_g * b_starting_h_idx * BT // NAtS_BLOCK_SIZE

    invovled_idx = tl.load(nats_block_indices + b_starting_h_idx)
...
# TODO this is only temporaily placed here!!!!
def complement_h(
        k: torch.Tensor,
        w: torch.Tensor,
        u: torch.Tensor,
    ...
    BT = chunk_size
    from nats.ops.utils import chunk_local_nats_cumsum, solve_tril_nats, prepare_chunk_offsets
    ...
    nats_block_delta_offsets = prepare_nats_chunk_offsets(n_nats_blocks, nats_block_size,
    ...
    starting_h_idx = compute_starting_idx_for_chunks(
        ...
        op_offset=offset_delta
    )

    h_full = k.new_empty(B, NT, H, K, V)

    #h = torch.empty(len(chunk_indices_delta_nats), GNAtS, K, V, device=k.device, dtype=k.dtype)
    final_state = k.new_empty(N, H, K, V, dtype=torch.float32) if output_final_state else None
    if nats_block_size < BT:
        # if nats_block_size is smaller than BT, then the values stored under h might not be divided by BT
        # This might result in an incorrect computational results. Hence, we need to ensure that new values
        # need to be saved here such that we can recover H_{BT * i_t} from H_{nats_chunk_size * i_nats}
        assert save_new_value is True
