...
def recompute_w_u_nats_fwd_kernel(
    ...
    u += (i_t_ * BT * GNAtS + i_gnats).to(tl.int64) * V + i_gnats * V
    ...
    p_beta = tl.make_block_ptr(beta, (T,), (H,), (o_t0,), (BT,), (0,))
    p_A = tl.make_block_ptr(A, (T -o_t0, BT), (H*BT, 1), (0, 0), (BT, BT), (1, 0))
    b_beta = tl.load(p_beta, boundary_check=(0,))
    b_A = tl.load(p_A, boundary_check=(0, 1))

    for i_v in range(tl.cdiv(V, BV)):
        p_v = tl.make_block_ptr(v, (T, V), (H*V, 1), (o_t0, i_v * BV), (BT, BV), (1, 0))
        p_u = tl.make_block_ptr(u, (T - o_t0, V), (stride_ut, 1), (0, i_v * BV), (BT, BV), (1, 0))
    ...
    if USE_G:
        g += bos * H + i_h
        p_g = tl.make_block_ptr(g, (T,), (GNAtS,), (o_t0,), (BT,), (0,))
        b_g = exp(tl.load(p_g, boundary_check=(0,)))

    for i_k in range(tl.cdiv(K, BK)):
        p_k = tl.make_block_ptr(k, (T, K), (H*K, 1), (o_t0, i_k * BK), (BT, BK), (1, 0))
        p_w = tl.make_block_ptr(w, (T - o_t0, K), (stride_wt, 1), (0, i_k * BK), (BT, BK), (1, 0))
        b_k = tl.load(p_k, boundary_check=(0, 1))
        b_kb = (b_k * b_beta[:, None]).to(b_k.dtype)
        if USE_G:
            b_kb *= b_g[:, None]
        if USE_GK:
            p_gk = tl.make_block_ptr(gk + (bos*H + i_h) * K, (T, K), (H*K, 1), (o_t0, i_k * BK), (BT, BK), (1, 0))
...
def prepare_wy_repr_bwd_nats_kernel(
    ...
    for i_k in range(tl.cdiv(K, BK)):
        p_k = tl.make_block_ptr(k + (bos*H + i_h) * K, (T, K), (H*K, 1), (o_t0, i_k * BK), (BT, BK), (1, 0))
        p_dk = tl.make_block_ptr(dk + (bos*H + i_h) * K, (T, K), (H*K, 1), (o_t0, i_k * BK), (BT, BK), (1, 0))
        p_dw = tl.make_block_ptr(dw, (T - o_t0, K), (stride_wt, 1), (0, i_k * BK), (BT, BK), (1, 0))
        b_k = tl.load(p_k, boundary_check=(0, 1))
    ...
    for i_v in range(tl.cdiv(V, BV)):
        p_v = tl.make_block_ptr(v + (bos*H + i_h) * V, (T, V), (H*V, 1), (o_t0, i_v * BV), (BT, BV), (1, 0))
        p_dv = tl.make_block_ptr(dv + (bos*H + i_h) * V, (T, V), (H*V, 1), (o_t0, i_v * BV), (BT, BV), (1, 0))
        p_du = tl.make_block_ptr(du, (T - o_t0, V), (stride_ut, 1), (0, i_v * BV), (BT, BV), (1, 0))
        b_v = tl.load(p_v, boundary_check=(0, 1))
    ...
    p_dg = tl.make_block_ptr(dg + (bos*H + i_h), (T,), (H,), (o_t0,), (BT,), (0,))
    p_dbeta = tl.make_block_ptr(dbeta + (bos*H + i_h), (T,), (H,), (o_t0,), (BT,), (0,))
