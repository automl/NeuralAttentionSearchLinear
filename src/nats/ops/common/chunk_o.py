...
@triton.autotune(
    configs=[
        ...
        for BTK in [64, ]  # TODO this can be 32 or 64?
...
@triton.heuristics({
    ...
    'N_NAtS_BLOCK_PER_TK': lambda args: triton.cdiv(args['BTK'], args['NAtS_BLOCK_SIZE']),
})
@triton.jit(do_not_specialize=['T'])
def chunk_bwd_kernel_dqsg(
        q,
        k,
        v,
        h,
        ...
        N_NAtS_BLOCK_PER_TK: tl.constexpr,
        N_NAtS_BLOCK_PER_T: tl.constexpr,
):
    # Different from the original implemetnation, we decompose this function into two parts.
    # the first part only compute the dq related information, including dq, dg1, ds@k
    # i_k, i_t, i_bh = tl.program_id(0), tl.program_id(1), tl.program_id(2)
    # i_b, i_h = i_bh // H, i_bh % H

    i_k, i_t_, i_gnats = tl.program_id(0), tl.program_id(1), tl.program_id(2)
    if BTK < BT:
        # in this case, we might need multiple
        i_t_nats_block = i_t_ // N_BTK_PER_BT
        i_t_bk_ = i_t_ % N_BTK_PER_BT
    else:
        i_t_nats_block = i_t_
        i_t_bk_ = 0
    off_bh_nats = tl.load(chunk_indices_op_nats + i_t_nats_block * 2).to(tl.int32)
    i_t = tl.load(chunk_indices_op_nats + i_t_nats_block * 2 + 1).to(tl.int32)
    ...
    k += (bos * H + i_h) * K
    dq += (bos * H + i_h) * K
    dnats_msk += (bos_nats * H + i_h) * N_TYPES + OFFSET_OP + i_k * B * TNAtS * H
    # dk += (bos * H + i_h) * K

    v += (i_t_ * BTK * GNAtS + i_gnats).to(tl.int64) * V
    dstq += (i_t_ * BTK * GNAtS + i_gnats).to(tl.int64) * K
    h += (i_t_ * GNAtS + i_gnats).to(tl.int64) * K * V
    ...
    # for delta rule only
    if USE_DW:
        # TODO we do not need w here?
        # w += (bos * H + i_h) * K
        w += (i_t_ * BTK * GNAtS + i_gnats).to(tl.int64) * K
        dw += (i_t_ * BTK * GNAtS + i_gnats).to(tl.int64) * K

    load_idx_block_start = (i_t * BT + i_t_bk_ * BTK) // NAtS_BLOCK_SIZE
    load_idx_block = load_idx_block_start + tl.arange(0, N_NAtS_BLOCK_PER_TK)
    nats_block_msk = load_idx_block < TNAtS

    o_t_nats = tl.load(nats_block_indices + load_idx_block * stride_block_types_t,
                       mask=nats_block_msk, other=TNAtS
                       ).to(tl.int32)
    starting_idx_next = (1 + i_t) * BT // NAtS_BLOCK_SIZE

    o_t_nats_scaled = o_t_nats * NAtS_BLOCK_SIZE
    o_kt = tl.reshape(
        o_t_nats_scaled[:, None] + tl.arange(0, NAtS_BLOCK_SIZE)[None, :], [BTK]
    )

    m_kt = o_kt < T

    b_idx_start_next = tl.load(nats_block_indices + starting_idx_next * stride_block_types_t,
                               mask=starting_idx_next < TNAtS, other=TNAtS
                               )
    # we gather the first value from o_kt as starting points
    # start_idx = tl.gather(o_kt, tl.zeros([1], dtype=tl.int32), 0).to(tl.int32)
    start_idx = tl.load(nats_block_indices + load_idx_block_start * stride_block_types_t, ).to(
        tl.int32) * NAtS_BLOCK_SIZE

    last_idx = min(b_idx_start_next * N_NAtS_BLOCK_PER_T, T).to(tl.int32)

    n_iters = tl.cdiv(last_idx - start_idx, BT)

    o_k = tl.arange(0, BK) + i_k * BK
    m_k = o_k < K

    b_k = tl.load(k + o_kt[:, None] * stride_kt + o_k[None, :],
                  mask=m_kt[:, None] & m_k[None, :],
                  other=0)

    off_vt = v + tl.arange(0, BTK)[:, None] * stride_vt
    o_v = tl.arange(0, BV)
    b_v1 = tl.load(off_vt + o_v,
                   mask=m_kt[:, None] & (o_v < V)[None, :],
                   other=0
                   )
    p_h = tl.make_block_ptr(h, (V, K), (1, V), (0, i_k * BK), (BV, BK), (0, 1))
    b_h1 = tl.load(p_h, boundary_check=(0, 1))

    if USE_G:
        dgq += i_k * all * H + bos * H + i_h
        dg += i_k * STRIDE_DG0 + i_t_ * BTK * GNAtS + i_gnats
        gq += bos * H + i_h
        g += i_t_ * BTK * GNAtS + i_gnats
        b_g = tl.load(g + tl.arange(0, BTK) * GNAtS, mask=m_kt, other=0)
        # b_dg_last = tl.zeros([1, ], dtype=tl.float32) if USE_G else None
        b_dg = tl.zeros([BTK], dtype=tl.float32)
    if USE_G_GAMMA:
        b_gamma = tl.load(g_gamma + i_h)
        b_g = b_gamma * (tl.arange(0, BT) + 1)
    ...
    if V > BV:
        ...
        b_v2 = tl.load(off_vt + o_v,
                       mask=m_kt[:, None] & (o_v < V)[None, :],
                       other=0
                       )
        p_h = tl.make_block_ptr(h, (V, K), (1, V), (BV, i_k * BK), (BV, BK), (0, 1))
        b_h2 = tl.load(p_h, boundary_check=(0, 1))
    if V > BV * 2:
        o_v = tl.arange(0, BV) + 2 * BV
        b_v3 = tl.load(off_vt + o_v,
                       mask=m_kt[:, None] & (o_v < V)[None, :],
                       other=0
                       )
        p_h = tl.make_block_ptr(h, (V, K), (1, V), (2 * BV, i_k * BK), (BV, BK), (0, 1))
        b_h3 = tl.load(p_h, boundary_check=(0, 1))
    if V > BV * 3:
        o_v = tl.arange(0, BV) + 3 * BV
        b_v4 = tl.load(off_vt + o_v,
                       mask=m_kt[:, None] & (o_v < V)[None, :],
                       other=0
                       )
        p_h = tl.make_block_ptr(h, (V, K), (1, V), (3 * BV, i_k * BK), (BV, BK), (0, 1))
        b_h4 = tl.load(p_h, boundary_check=(0, 1))

    b_dstq = tl.zeros([BTK, BK], dtype=tl.float32)
    b_dnats_msk = tl.zeros([BTK], dtype=tl.float32)

    for i in range(n_iters):
        # b_dk = tl.zeros([BT, BK], dtype=tl.float32)
        # b_dw = tl.zeros([BT, BK], dtype=tl.float32) if USE_DW else None
        o_qt_start = i * BT + start_idx

        p_do = tl.make_block_ptr(do, (last_idx, V), (H * V, 1), (o_qt_start, 0), (BT, BV), (1, 0))
        b_do = tl.load(p_do, boundary_check=(0, 1))

        b_ds = tl.dot(b_do, tl.trans(b_v1))
        b_dq = tl.dot(b_do, b_h1.to(b_do.dtype))

        if V > BV:
            p_do = tl.make_block_ptr(do, (last_idx, V), (H * V, 1), (o_qt_start, BV), (BT, BV), (1, 0))
            b_do = tl.load(p_do, boundary_check=(0, 1))
            b_ds += tl.dot(b_do, tl.trans(b_v2))
            b_dq += tl.dot(b_do, b_h2.to(b_do.dtype))

        if V > 2 * BV:
            p_do = tl.make_block_ptr(do, (last_idx, V), (H * V, 1), (o_qt_start, 2 * BV), (BT, BV), (1, 0))
            b_do = tl.load(p_do, boundary_check=(0, 1))
            b_ds += tl.dot(b_do, tl.trans(b_v3))
            b_dq += tl.dot(b_do, b_h3.to(b_do.dtype))

        if V > 3 * BV:
            p_do = tl.make_block_ptr(do, (last_idx, V), (H * V, 1), (o_qt_start, 3 * BV), (BT, BV), (1, 0))
            b_do = tl.load(p_do, boundary_check=(0, 1))
            b_ds += tl.dot(b_do, tl.trans(b_v4))
            b_dq += tl.dot(b_do, b_h4.to(b_do.dtype))

        p_q = tl.make_block_ptr(q, (last_idx, K), (H * K, 1), (o_qt_start, i_k * BK), (BT, BK), (1, 0))
        b_q = tl.load(p_q, boundary_check=(0, 1))

        p_dq = tl.make_block_ptr(dq, (last_idx, K), (H * K, 1), (o_qt_start, i_k * BK), (BT, BK), (1, 0))

        o_qt = o_qt_start + tl.arange(0, BT)
        m_qt = o_qt < last_idx

        if COMPUTE_INCOMPLETE_BLOCK_SCORES:
            m_A = (o_qt[:, None] >= o_kt[None, :])
        else:
            m_A = (o_t_nats_scaled[None, :] + NAtS_BLOCK_SIZE - 1) <= o_qt[:, None]
            m_A = tl.reshape(
                tl.broadcast_to(m_A[:, :, None], [BT, N_NAtS_BLOCK_PER_TK, NAtS_BLOCK_SIZE, ]), [BT, BTK])

        m_A = m_A & (m_kt[None, :] & m_qt[:, None])
        if USE_G:
            # g += bos * H + i_h
            # dg += bos * H + i_h
            p_gq = tl.make_block_ptr(gq, (last_idx,), (H,), (o_qt_start,), (BT,), (0,))
            b_gq = tl.load(p_gq, boundary_check=(0,))
            # b_g_last = tl.load(g + (min(i_t * BT + BT, T) - 1) * H)
            # b_dg_last *= tl.exp(b_g_last)

            b_dq = b_dq * tl.exp(b_gq)[:, None] * scale
            b_dgq = tl.sum(b_dq * b_q, axis=1)
            b_ds = tl.where(m_A, b_ds, 0) * scale
            b_dnats_msk += tl.sum(b_ds, axis=0)

            b_ds = b_ds * tl.exp(b_gq[:, None] - b_g[None, :])
            b_ds2 = b_ds * tl.dot(b_q, tl.trans(b_k))
            b_dgq += tl.sum(b_ds2, axis=1)
            b_dg -= tl.sum(b_ds2, axis=0)
            p_dgq = tl.make_block_ptr(dgq, (last_idx,), (H,), (o_qt_start,), (BTK,), (0,))
            tl.store(p_dgq, b_dgq.to(p_dgq.dtype.element_ty), boundary_check=(0,))

            b_ds = b_ds.to(b_k.dtype)
            # [BT, BK]
            b_dq += tl.dot(b_ds, b_k)

            b_dstq += tl.dot(tl.trans(b_ds), b_q)

            # (SY 09/21) revcumsum in a separate kernel due to strange triton compiler issue
            # b_dg = tl.dot(tl.where(o_t[:, None] <= o_t[None, :], 1., 0.), b_dg, allow_tf32=False) + b_dg_last)
            # b_dg = tl.where(o_t < min(i_t * BT + BT, T) - 1, b_dg, b_dg + b_dg_last) # this moves to d_k
    ...
    # we need to store b_dstq for bk
    tl.store(dstq + tl.arange(0, BTK)[:, None] * K * GNAtS + o_k[None, :],
             b_dstq.to(dstq.dtype.element_ty),
             mask=m_kt[:, None] & m_k[None, :],
             )

    b_dnats_msk = tl.sum(tl.reshape(b_dnats_msk, [N_NAtS_BLOCK_PER_TK, NAtS_BLOCK_SIZE]), -1) / NAtS_BLOCK_SIZE

    tl.store(dnats_msk + load_idx_block * H * N_TYPES,
             b_dnats_msk.to(dnats_msk.dtype.element_ty),
             mask=nats_block_msk)
    if USE_G:
        tl.store(dg + tl.arange(0, BTK) * GNAtS,
...
def chunk_bwd_kernel_dkwg(
    ...
    if chunk_is_delta:
        ...
        if COMPUTE_DNATS_FOR_INCOMPLETE_SCORES:
            b_nats_opt += tl.sum((b_dk_virtual * b_k)) / BT
            tl.store(dnats + i_t * H, b_nats_opt.to(dnats.dtype.element_ty))
