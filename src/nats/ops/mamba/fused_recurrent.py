...
def fused_recurrent_mamba_fwd_nats_kernel(
    q,
    k,
    v,
    g,
    ...
    # the first iteration
    if ONLY_UPDATE_HIDDEN_STATES:
        is_mamba = tl.load(nats_block_types).to(tl.int1)
        if not is_mamba:
            if USE_G:
    ...
    if not ONLY_UPDATE_H:
        if not ONLY_WITH_CURRENT_CHUNK:
            ...
            if n_iter1 == NATS_BLOCK_SIZE - b_ntokens_in_current_block:
                is_mamba = tl.load(nats_block_types)
                if DECAY_FOR_NON_MAMBA_BLOCKS:
                    b_h = tl.where(is_mamba, b_h, b_hcur * tl.exp(b_g_cumsum))
                else:
                    b_h = tl.where(is_mamba, b_h, b_hcur)
...
def fused_recurrent_mamba_nats_fwd(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    ...
    fused_recurrent_mamba_fwd_nats_kernel[grid](
        q=q,
        k=k,
        v=v,
        g=g,
...
class FusedRecurrentNAtSFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        o, final_state, initial_state_in_current_block = fused_recurrent_mamba_nats_fwd(
            q=q,
            k=k,
            v=v,
            g=g,
...
def test_recurrent_fwd():
    ...
    for i_chunk in range(T_NAtS):
        if i_chunk == 0:
            ...
            #g_in = g[:, t_current:t_end]
            t_current = t_end
        head_dim = v_in.shape[-1]
        d_state = k_in.shape[-1]
        state = state_tmp.transpose(2,3).contiguous()

        for i in range(q_in.shape[1]):

            o1,  dA, dB, x = selective_state_update_ref(
                state=state, x=v_in[:,i].contiguous(), 
                dt=dt_in[:,i,][..., None].expand(-1, -1, head_dim).contiguous(),
                A=A[:, None, ...][:, :, None].expand(-1, head_dim, d_state).to(dtype=torch.float32),
                B=k_in[:, i].contiguous(),
                C=q_in[:, i].contiguous(), D=None, z=None, 
                dt_bias=dt_bias[:, None, ...].expand(-1, head_dim), dt_softplus=True,
            )
            final_state1 = copy.deepcopy(state).transpose(2,3)
    ...
    o1 = torch.stack(all_output, dim=1)
    final_state_chunkstart1 = state_chunk_start

    dt_in = dt + dt_bias
    #dt_in = torch.where(dt_in<20., F.softplus(dt_in), dt_in)
    dt_in = F.softplus(dt_in)
    g = A[None, None, :] * dt_in
    k1 = (k * dt_in[..., None]).to(k.dtype)

    o, final_state, initial_state_in_current_block = fused_recurrent_mamba(
        q=q,
        k=k1,
        v=v,
        g=g,
        ...
        initial_state_current=copy.deepcopy(h0_chunk_start),
        offset_op=DELTA_IDX,
        output_final_state=True,
        cu_seqlens=None,
    ...
    h0 * torch.exp(g[:,0][..., None, None]) + k1.permute(0,2,3,1) @ v.permute(0,2,1,3)
