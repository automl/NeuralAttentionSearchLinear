...
def parallel_nats_attn_fwd_kernel(q, k, v, o,  # Q, O are of shape [B, T, H, DQ]
    ...
    if STORE_MSK:
        msk += (T.to(tl.int64) * T * HQ) * i_b + (T.to(tl.int64) * T) * i_hq
        ...
        if (NAtS_BLOCK_SIZE > BT and COMPUTE_INCOMPLETE_CHUNK_SCORES) or STAGE == 1:
            ...
            if i_s0 < T:  # meaning that the last block is also attention block, we need to do a further computation
                ...
                for i_iter_nats in range(0, b_n_iters_in_nats_block):
                    ...
                    b_o = b_o * b_r[:, None] + tl.dot(b_p.to(b_q.dtype), b_v)
                    b_mp = b_m
    if STAGE & 2 and (not COMPUTE_INCOMPLETE_CHUNK_SCORES and BT > NAtS_BLOCK_SIZE):
        ...
        for i_iter in tl.range(0, b_n_iters, ):
            ...
            if is_attn_block or COMPUTE_INCOMPLETE_CHUNK_SCORES:
                ...
                if COMPUTE_INCOMPLETE_CHUNK_SCORES:
                    ...
                    m_s = tl.where(is_attn_block, m_s, m_s & o_q[:, None] < ((i_nats_block + 1) * NAtS_BLOCK_SIZE))
                else:
...
@triton.jit
def parallel_mixed_attn_bwd_kernel_dq(q, k, v,  # Q, O are of shape [B, T, H, DQ]
                                      g_cumsum, lse, delta,
                                      do, dq, dg_cumsum,
            ...
            for i_iter in tl.range(0, b_n_iters, ):
                ...
                if is_attn_block or COMPUTE_INCOMPLETE_CHUNK_SCORES:
                    ...
                    if COMPUTE_INCOMPLETE_CHUNK_SCORES:
                        ...
                        m_s = tl.where(is_attn_block, m_s, m_s & ~m_s_lower) & m_k[None, :]
                    else:
...
def parallel_attn_bwd_kernel_dkdv(q, k, v,  # Q, O are of shape [B, T, H, DQ]
    ...
    # Our current solution is to consider H as 1, i.e., we need to transpose the input data to [B * H, T, 1, D]
    nats_block_types += (bos_nats * HNAtS + i_hnats) * N_TYPES + OFFSET_ATTN
    dnats_block_types += (bos_nats * N_CHUNK_PER_NAtS_BLOCK * HQ + i_h) * N_TYPES + OFFSET_ATTN
    nats_block_indices += (bos_nats * HNAtS + i_hnats) * N_TYPES + OFFSET_ATTN
    ...
    # TODO Check if we really need different types for different heads
    stride_block_types_t = N_TYPES * HNAtS
    stride_dblock_attn_t = N_TYPES * HQ
    stride_qt = K * HQ
    stride_ot = V * HQ
    stride_lset = HQ
...
def mixed_parallel_attn_fwd(
        q: torch.Tensor,
        k: torch.Tensor,
        v: torch.Tensor,
        nats_nats_block_types: torch.Tensor,
        nats_block_indices: torch.LongTensor,
        g_cumsum: torch.Tensor,
        scale: float,
        NAtS_block_size: int,
        offset_attn: int = 0,
        compute_incmplete_chunk_scores: bool = False,
        chunk_size: int = 128,
    ...
    TNAtS, HNAtS, N_TYPES = nats_nats_block_types.shape[1:]
    HQ = q.shape[2]
    G = HQ // H
    GNAtS = HQ // HNAtS
    ...
    NK = triton.cdiv(BK, BK)
    NV = triton.cdiv(BV, BV)
    ...
    parallel_nats_attn_fwd_kernel[grid](
        ...
        nats_block_types=nats_nats_block_types, nats_block_indices=nats_block_indices,
        ...
        STAGE=stage,
        COMPUTE_INCOMPLETE_CHUNK_SCORES=compute_incmplete_chunk_scores,
        N_TYPES=N_TYPES,
...
def mixed_parallel_attn_bwd(
        ...
        nats_nats_block_types: torch.Tensor,
        nats_block_indices: torch.LongTensor,
        n_nats_chunks: torch.LongTensor,
        g_cumsum: torch.Tensor,
        lse: torch.Tensor,
        do: torch.Tensor,
        scale: float = None,
    ...
    dnats_block_types = torch.zeros(B, TNAtS * (triton.cdiv(NAtS_block_size, BT)), HQ, N_TYPES,
    ...
    parallel_mixed_attn_bwd_kernel_dq[grid](q=q, k=k, v=v,
                                            g_cumsum=g_cumsum, lse=lse, delta=delta,
                                            do=do, dq=dq, dg_cumsum=dg_cumsum,
                                            nats_block_types=nats_nats_block_types,
    ...
    chunk_indices_attn_nats = prepare_nats_block_indices(n_nats_chunks, NAtS_block_size, BT, OFFSET_ATTN)
    ...
    if NAtS_block_size > BT:
        dnats_block_types = reduce(
            dnats_block_types, 'b (t gt) (h g) v -> b t h v',
            g=GNAtS, gt=triton.cdiv(NAtS_block_size, BT), reduction='sum')

    else:
        dnats_block_types = reduce(dnats_block_types, 'b t (h g) v -> b t h v', g=GNAtS, reduction='sum')
...
@torch.compile
class ParallelMixedAttentionFunction(torch.autograd.Function):
    @staticmethod
    @contiguous
    @autocast_custom_fwd
    def forward(ctx, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor,
                ...
                nats_nats_block_types: torch.Tensor,
                nats_block_indices: torch.LongTensor,
                n_nats_chunks: torch.Tensor,
                scale, cu_seqlens, cu_seqlens_nats,
                NAtS_block_size: int,
                offset_attn: int = 0, compute_incmplete_chunk_scores: bool = False, is_causal: bool = True,
                store_msk: bool=False
                ):
        ctx.dtype = q.dtype
        ...
        o, lse, msk = mixed_parallel_attn_fwd(
            q, k, v, nats_nats_block_types, nats_block_indices,
            g_cumsum, scale, NAtS_block_size=NAtS_block_size,
            offset_attn=offset_attn, compute_incmplete_chunk_scores=compute_incmplete_chunk_scores,
            is_causal=is_causal, store_msk=store_msk,
        )
        ctx.save_for_backward(q, k, v, nats_nats_block_types, nats_block_indices, n_nats_chunks, o, g_cumsum, lse)
        ...
        ctx.compute_incmplete_chunk_scores = compute_incmplete_chunk_scores
    ...
    def backward(ctx, do, dmsk):
        q, k, v, nats_nats_block_types, nats_block_indices, n_nats_chunks, o, g_cumsum, lse = ctx.saved_tensors
        dq, dk, dv, dg, d_chunk_type = mixed_parallel_attn_bwd(
            q, k, v, o, nats_nats_block_types=nats_nats_block_types,
            nats_block_indices=nats_block_indices,
            n_nats_chunks=n_nats_chunks,
            g_cumsum=g_cumsum,
            lse=lse,
            do=do,
            scale=ctx.scale,
            ...
            compute_incmplete_chunk_scores=ctx.compute_incmplete_chunk_scores,
            cu_seqlens=ctx.cu_seqlens,
            cu_seqlens_nats=ctx.cu_seqlens_nats,
            is_causal=ctx.is_causal,
        )
        return dq.to(q), dk.to(k), dv.to(v), dg, d_chunk_type.to(
            nats_nats_block_types), None, None, None, None, None, None, None, None, None


def parallel_mixed_attn(
    ...
    where nats_nats_block_types == Attention to compute the attention output.
    the nats_nats_block_types could be determined by chunks, i.e., we determine the types for the tokens for every
    ...
    after we have observed the last chunk. compute_incmplete_chunk_scores determines if we would like to
    ...
    Args:
        ...
        compute_incmplete_chunk_scores (bool):
    ...
    o, msk = ParallelMixedAttentionFunction.apply(q, k, v, g,
                                             nats_nats_block_types, nats_block_indices, n_nats_chunks,
                                             scale, cu_seqlens,
                                             cu_seqlens_nats, NAtS_block_size, offset_attn,
                                             compute_incmplete_chunk_scores, is_causal
                                             )
    return o, msk
...
def test_mixed_attns():
    ...
    n_nats_chunks = attn_types.int().sum(1)
    ...
    from nats.models.transformer.utils import repeat_kv, repeat_masks
    """
