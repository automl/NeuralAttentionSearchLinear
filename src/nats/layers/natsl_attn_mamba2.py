...
from nats.ops.mixed_ops.mixed_attn_mamba import nats_mixed_attn_mamba, nats_mixed_attn_mamba_chunk_inference, nats_mixed_attn_mamba_recurrent
...
class NeuralAttentionSearchLinearAttnMamba(nn.Module):
    """
    ...
    def __init__(
            ...
            norm_eps: float = 1e-5,
            **kwargs
    ) -> NeuralAttentionSearchLinearAttnMamba:
        super().__init__()
        ...
        self.q_norm2 = L2Norm(eps=norm_eps)
        self.k_norm2 = L2Norm(eps=norm_eps)
        ...
        self.decay_for_non_mamba_blocks = decay_for_non_mamba_blocks
    ...
    def forward(
            ...
            cache_params: Optional[NAtSCache] = None,
        ...
        if cache_params is not None:
            use_cache = True
            if self.layer_idx in cache_params.nats_cache:
                nats_cache = cache_params.nats_cache[self.layer_idx]
            else:
                cache_params.add_natsl_layer_cache(
                    self.layer_idx, 0, n_tokens_in_nats_block=0, n_attn_blocks=None,
                    nats_block_size=self.nats_block_size, op_for_incomplete_chunk=self.ops_for_incomplete_chunks,
                )
        if cache_params is not None and self.layer_idx in cache_params.nats_cache:
            nats_cache = cache_params.nats_cache[self.layer_idx]
        ...
        if nats_cache is not None:
            ...
            k, v, beta, g = nats_cache.update_lattn_cache((k,v), None, g, mode, self.ops_for_incomplete_chunks)

            recurrent_state_mamba_block_start = nats_cache.recurrent_state_block_start
            recurrent_state_mamba = nats_cache.recurrent_state
            if nats_cache.g_cumsum is not None:
                g_cumsum = nats_cache.g_cumsum
            else:
                g_cumsum = torch.zeros(1, 1, g.shape[2], device=g.device, dtype=g.dtype)
        else:
            recurrent_state_mamba = None
            recurrent_state_mamba_block_start = None
            g_cumsum = 0
        ...
        if mode == 'chunk' and self.training:
            ...
            o_mamba, o_attn, recurrent_state_mamba = nats_mixed_attn_mamba(
                q_attn=q_attn, k_attn=k_attn, v_attn=v_attn,
                q_lattn=q, k_lattn=k, v_lattn=v,
                initial_state_mamba=recurrent_state_mamba,
                g=g,
                ...
                output_final_state_mamba=use_cache,
                ...
                decay_for_non_mamba_blocks=self.decay_for_non_mamba_blocks,
            ...
            nats_op_types[:, -1] = 1
            n_nats_blocks = nats_op_types.int().sum(1)
            o_mamba, o_attn, recurrent_state_mamba, recurrent_state_mamba_block_start = nats_mixed_attn_mamba_chunk_inference(
                q_attn=q_attn, k_attn=k_attn, v_attn=v_attn,
                q_lattn=q, k_lattn=k, v_lattn=v,
                initial_state_mamba_start=recurrent_state_mamba_block_start,
                g=g,
                nats_block_types=nats_op_types,
            ...
            attn_msk = nats_cache.generate_msk(n_data= q_len, nats_block_types=nats_op_types,
                                               compute_incomplete_chunk=self.ops_for_incomplete_chunks != 'mamba',
                                               )
            # recurrent_state_gated_delta_ = recurrent_state_gated_delta.clone()
            o_mamba, o_attn, recurrent_state_mamba, recurrent_state_mamba_block_start = nats_mixed_attn_mamba_recurrent(
                q_attn=q_attn, k_attn=k_attn, v_attn=v_attn,
                q_lattn=q, k_lattn=k, v_lattn=v,
                attn_msk=attn_msk,
                initial_state_mamba=recurrent_state_mamba,
                initial_state_mamba_chunk_start=recurrent_state_mamba_block_start,
                g=g, g_cumsum=g_cumsum, 
                scale_attn=k_attn.shape[-1] ** -0.5, scale_lattn=1,
                nats_block_types=nats_op_types,
        ...
        if nats_cache is not None:
            ...
            nats_cache.update(
                recurrent_state=recurrent_state_mamba,
                recurrent_state_block_start=recurrent_state_mamba_block_start,
        ...
        if self.use_gate:
            ...
            if self.outputs_are_wighted:
                ...
                o = self.o_norm(o_mamba.view(o_attn.shape), o_attn, g, o_weights)
            else:
                o = self.o_norm(o_mamba, o_attn.view(o_mamba.shape),g)
        else:
...
def test_mixed_attn():
    ...
    layer = NeuralAttentionSearchLinearAttnMamba(hidden_size=1024, head_dim=128, num_heads=8, num_attn_heads=16,
                                        expand_v=1,
    ...
    class Cache_params:
        def __init__(self, ):
            #self.nats_cache = {0: NAtSLayerCache(op_for_incomplete_chunk=ops_for_incomplete_chunks)}
            self.nats_cache = {}
        def add_natsl_layer_cache(self,
            ...
            self.nats_cache[layer_idx]=NAtSLayerCache(seen_tokens,
    ...
    if nt1 >0:
        ...
        out21 = layer(input_data[:, :nt1], cache_params=cache, use_cache=True)
        out2[:,:nt1] = out21

    from tqdm import tqdm
    for i in tqdm(range(nt2)):
        o_current = layer(input_data[:, [nt1+i]],  cache_params=cache, use_cache=True)
